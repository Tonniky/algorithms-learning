...
public class Problem_09_EditCost {
	...
	public static int minCost1(String str1, String str2, int ic, int dc, int rc) {
		...
		char[] chs1 = str1.toCharArray();   //替换中间值
		char[] chs2 = str2.toCharArray();
		int row = chs1.length + 1;        //行的数目
		int col = chs2.length + 1;        //列的数目
		int[][] dp = new int[row][col];   //新建dp
		for (int i = 1; i < row; i++) {   //边界初始化
			dp[i][0] = dc * i;
		}
		for (int j = 1; j < col; j++) {   //边界初始化
			dp[0][j] = ic * j;
		}
		for (int i = 1; i < row; i++) {
			for (int j = 1; j < col; j++) {
